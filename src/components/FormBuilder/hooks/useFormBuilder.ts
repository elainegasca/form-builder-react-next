...
import { TemplateType, FormLayoutComponentsType, FormLayoutComponentChildrenType, FormLayoutComponentContainerType } from "../../../types/FormTemplateTypes";
...
const useFormBuilder = (props: useFormBuilderProps) => {
  ...
  const [formLayoutComponents, setFormLayoutComponents] = useState<FormLayoutComponentsType[]>(props.template.formLayoutComponents);
  const [selectedControl, setSelectedControl] = useState<undefined | FormLayoutComponentContainerType | FormLayoutComponentChildrenType>(undefined);
  ...
  const deleteContainer = (containerId: string) => {
    if (confirm("Are you sure you want to delete container?")) {
      ...
      setFormLayoutComponents(newState);
      setSelectedControl((prev) => (prev && (prev.id === containerId || (prev as FormLayoutComponentChildrenType).containerId === containerId) ? undefined : prev));
    }
  };
  ...
  const moveControlFromSide = (item: FormLayoutComponentChildrenType, { containerId, position }: FormLayoutComponentChildrenType) => {
  ...
  const moveControl = (item: FormLayoutComponentChildrenType, dragIndex: number, hoverIndex: number, containerId: string) => {
    if (item === undefined) {
      return;
    }
    let componentsCopy: FormLayoutComponentsType[] = JSON.parse(JSON.stringify(formLayoutComponents));
  ...
  const checkIfControlsInContainer = () => {
    for (let i = 0; i < formLayoutComponents.length; i++) {
      ...
      if (componentChildren.length === 0) {
        showModalStrip("danger", "You need to have controls inside containers before updating.", 5000);
        return false;
      }
    }
    return true;
  ...
  const publishForm = () => {
    ...
    dispatch(saveTemplate(currentTemplate))
      .unwrap()
      .then((newTemplate) => {
        console.log("this is what would be published", newTemplate);
  ...
  const saveForm = () => {
    ...
    if (JSON.stringify(currentTemplate.formLayoutComponents) === JSON.stringify(formLayoutComponents)) {
      showModalStrip("info", "No Change in current & previous saved version.", 5000);
      return;
    }
    currentTemplate.formLayoutComponents = formLayoutComponents;
