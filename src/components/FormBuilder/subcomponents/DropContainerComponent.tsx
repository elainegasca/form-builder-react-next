...
interface DropContainerComponentProps {
  ...
  name?: string;
  index?: number;
  handleItemAdded?: (item: FormLayoutComponentChildrenType | FormLayoutComponentContainerType, containerId?: string) => void;
  ...
  moveControl?: (item: FormLayoutComponentChildrenType, dragIndex: number, hoverIndex: number, containerId: string) => void;
}

const DropContainerComponent: FunctionComponent<DropContainerComponentProps> = (props) => {
  const { accept, layout, childrenComponents, index, deleteContainer, deleteControl, selectControl, selectedControl, handleItemAdded, moveControl } = props;
  ...
  return (
    <>
      <div className="col-12 container-drop" ref={drop} style={{ backgroundColor, borderRadius: "9px", border }}>
        {accept === FormItemTypes.CONTAINER ? (
          <>
            <div className="d-flex justify-content-center align-items-center" style={{ minHeight: "90px" }}>
              <Button
                onClick={() => {
                  if (handleItemAdded) {
        ...
        {accept === FormItemTypes.CONTROL ? (
          <>
            ...
            <div className="d-flex flex-column justify-content-center align-items-center" style={{ minHeight: "20vh", position: "relative" }}>
                  ...
                  {childrenComponents?.map((component, ind) => {
                    return (
                      <ControlViewComponent
                        ...
                        index={ind}
                        moveControl={(item, dragIndex, hoverIndex, containerId) => {
                          if (moveControl) {
                            moveControl(item, dragIndex, hoverIndex, containerId);
                          }
                        }}
