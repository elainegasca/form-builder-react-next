...
const ManageItemsListComponent: FC<ManageItemsListComponentProps> = (props) => {
  ...
  const onSubmit: React.MouseEventHandler<HTMLInputElement> = (event: any) => {
  ...
  return (
    <>
      <div>
        <TextField label="Item Name" name="newItem" value={itemName} onChange={handleChange} style={{ minWidth: "100%" }} />
        <input className="btn btn-light btn-shadow m-t-20 m-r-10" value={editMode ? "Edit Item" : "Add Item"} type="button" onClick={onSubmit} />
        {editMode && <input className="btn btn-light btn-shadow m-t-20 m-r-10" value="Cancel" type="button" onClick={cancelEditing} />}
        <List component="nav">
          {items?.map((item, ind) => {
            return (
